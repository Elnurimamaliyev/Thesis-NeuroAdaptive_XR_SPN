...
#if ENABLE_LEGACY_INPUT_MANAGER
            var pressed = Input.GetKeyDown(gazeClickKey) || OVRInput.GetDown(joyPadClickButton) || Input.GetMouseButtonDown(0);
            var released = Input.GetKeyUp(gazeClickKey) || OVRInput.GetUp(joyPadClickButton) || Input.GetMouseButtonUp(0);
