...
partial class OculusBuildSamples
{
    ...
    private static void InitializeBuild(string identifier) {
        ...
        PlayerSettings.SetScriptingBackend (GetNamedBuildTarget(BuildTargetGroup.Android), ScriptingImplementation.IL2CPP);
        PlayerSettings.SetArchitecture (GetNamedBuildTarget(BuildTargetGroup.Android), 1); //0 - None, 1 - ARM64, 2 - Universal
        PlayerSettings.Android.targetArchitectures = AndroidArchitecture.ARM64;
        EditorUserBuildSettings.androidBuildSystem = AndroidBuildSystem.Gradle;
        QualitySettings.antiAliasing = 4;
        PlayerSettings.SetApplicationIdentifier(GetNamedBuildTarget(BuildTargetGroup.Android), identifier);
    }
    ...
    // Add the helper method for conversion
    ...
    // Updated method to handle Android project settings without ANDROID_PACKAGE_NAME dependency
    static void UpdateAndroidProject()
    {
        // Update API calls to use the modern API without requiring ANDROID_PACKAGE_NAME
        PlayerSettings.SetScriptingBackend(GetNamedBuildTarget(BuildTargetGroup.Android), ScriptingImplementation.IL2CPP);
        PlayerSettings.SetArchitecture(GetNamedBuildTarget(BuildTargetGroup.Android), 1); // 0 - None, 1 = ARM64, 2 = Universal
        
        // Skip the package name setting since you don't have an Android project
        // This line is what's causing the error since ANDROID_PACKAGE_NAME doesn't exist
        
        // Continue with any other Android-specific settings
        UnityEngine.Debug.Log("Android project settings updated with modern API calls");
    }
}
